...
char* GBKtoUTF8(char* strGbk)
{
    ...
    if (!strTemp)
        return NULL;
    strcpy_s(strTemp, sizeof(char) * strlen(strUtf8) + 1, strUtf8);//此时的strTemp是UTF-8编码  
    free(strUnicode);
    free(strUtf8);
    return strTemp;
}
...
char* UTF8toGBK(char* strUtf8)
{
    ...
    char* strGbk = (char*)malloc(len + 1);//len=3 本来为2，但是char*后面自动加上了\0 
    if (!strGbk)
        return NULL;
    memset(strGbk, 0, len + 1);
    WideCharToMultiByte(CP_ACP, 0, strUnicode, -1, strGbk, len + 1, NULL, NULL);

    char* strTemp = (char*)malloc(sizeof(char) * strlen(strGbk) + 1);//此时的strTemp是GBK编码  
    if (!strTemp)
        return NULL;
    strcpy_s(strTemp, sizeof(char) * strlen(strGbk) + 1, strGbk);
    free(strUnicode);
    free(strGbk);
    return strTemp;
}