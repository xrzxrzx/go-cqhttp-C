#pragma once
#ifndef _CQHTTP_API_H_
#define _CQHTTP_API_H_
...
#define API_SEND_GROUP_MSG_FORM		"GET /send_group_msg?group_id=%u&message=%s&auto_escape=%d HTTP/1.1\r\nHost: 127.0.0.1:5700\r\nConnection: keep-alive\r\n\r\n"
...
#define API_GET_MSG_RECV			"%*[^{]{\"data\":{\"group\":%[^,],\"group_id\":%u,\"message\":\"%[^\"]\",\"message_id\":%d,\"message_id_v2\":\"%[^\"]\",\"message_seq\":%d,\"message_type\":\"%[^\"]\",\"real_id\":%d,\"sender\":{\"nickname\":\"%[^\"]\",\"user_id\":%lu},\"time\":%d},\"retcode\":%d,\"status\":\"%[^\"]\"}"
...
typedef struct
{
	struct
	{
		...
		char level[50];			//成员等级
		char role[10];			//角色
		int unfriendly;			//是否不良记录成员
		char title[50];			//专属头衔
		long title_expire_time;	//专属头衔过期时间戳
		int card_changeable;	//是否允许修改群名片
